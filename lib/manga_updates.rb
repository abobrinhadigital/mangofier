...
class MangaUpdates
  ...
  # Busca lançamentos com metadados de ranking (para Hype Hunter)
  # Retorna array com series_id, title, rank_week (old/current), rating, etc.
  def fetch_releases_with_ranking(start_date)
    login if @token.nil?

    page = 1
    max_pages = 1 # 100 releases por teste
    series_ids = []

    puts "📡 [MU] Buscando lançamentos desde #{start_date}..."

    # Fase 1: Coleta IDs das séries com lançamentos
    loop do
      ...
      break unless response.success? && response.body[:results]

      results = response.body[:results]
      break if results.empty?

      puts "   📄 Página #{page}: #{results.length} releases"

      results.each do |item|
        series_id = item.dig(:metadata, :series, :series_id)
        series_ids << series_id if series_id
      end

      break if results.length < 100
    ...
    series_ids.uniq!
    series_ids = series_ids.first(30) # Limita para performance
    puts "📚 [MU] #{series_ids.length} séries únicas para verificar rank"

    # Fase 2: Busca detalhes de cada série (rank, rating, etc)
    all_results = []
    series_ids.each_with_index do |series_id, idx|
      print "\r   🔍 Analisando série #{idx + 1}/#{series_ids.length}... "
      $stdout.flush

      series_data = fetch_series_details(series_id)
      next unless series_data

      rank = series_data[:rank]
      next unless rank && rank[:position] && rank[:old_position]

      rank_week = rank[:position][:week]
      old_rank_week = rank[:old_position][:week]
      next unless rank_week && old_rank_week

      delta = old_rank_week - rank_week
      next if delta <= 0 # Só interessa quem SUBIU
      ...
      all_results << {
        ...
        bayesian_rating: series_data[:bayesian_rating],
        rank_old: old_rank_week,
        rank_current: rank_week,
        delta_week: delta,
        chapter: nil,
        time_added: nil
      }

      sleep(0.1) if idx < series_ids.length - 1 # Rate limiting rápido
    end

    puts "\n📡 [MU] #{all_results.length} séries com delta positivo"
    all_results
  end
